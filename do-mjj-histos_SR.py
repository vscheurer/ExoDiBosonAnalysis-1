...
argv = sys.argv
parser = OptionParser()   
parser.add_option("-L", "--lumi", dest="lumi", default=36814,
                              help="Set lumi")
...
data_filename = "Data_VVdijet_SB_36814ifb.root"
...
#filename = "QCD_pythia8_VVdijet_SB.root"
filename = "QCD_madgraph_pythia8_qVdijet_SB.root"
...
if path.find("Summer16")!=-1:
    name +="_summer16"
if opts.dataSB:
    name = "RerecoData_VVdijet" #_test40GeV"
    if data_filename.find("qV")!=-1:
        name = "ReRecoData_qVdijet"
...
for root, _, files in os.walk(path):
  for f in files:
    ...
    if f.find(filename)!=-1:
        ...
        if fullpath.find("QCD")!=-1 or opts.signal!="":
            lumi = opts.lumi
        if fullpath.find("Data")!=-1:
            lumi = 1.
        if fullpath.find("SB")!=-1:
            name+= "_SB"  
        if fullpath.find("BulkWW")!=-1:
            name= "BulkToWW_13TeV_"+str(opts.mass)+"GeV"
        if fullpath.find("BulkZZ")!=-1:
            name = "BulktoZZ_13TeV_"+str(opts.mass)+"GeV"
        if fullpath.find("ZprimeWW")!=-1:
            name = "ZprimeToWW_13TeV_"+str(opts.mass)+"GeV"
        if fullpath.find("WprimeWZ")!=-1:
            name = "WprimeToWZ_13TeV_"+str(opts.mass)+"GeV"
        ...
        for event in intree:
            ...
            if name.find("SB")!=-1 and (name.find("q")!=-1 or fullpath.find("Qstar")!=-1):
                    ...
                    if ( mw_min_SB_qVdown > event.jet_puppi_softdrop_jet1 or mw_min_SB_qVdown > event.jet_puppi_softdrop_jet2):
                        ...
                        #continue
                    #if ( event.jet_puppi_tau2tau1_jet1 <= 0.40 or event.jet_puppi_tau2tau1_jet2<= 0.4):
                        #qVHP.Fill(event.MVV,event.weight)
                        #nEvents[0]+=1
                    #if ( 0.40 <event.jet_puppi_tau2tau1_jet1 <= 0.75 or 0.40 <event.jet_puppi_tau2tau1_jet1 <= 0.75):
                    ...
                    #===================== qV try new sideband definition: leave q jet as it is, and require V jet to have a mass between 105-200 GeV================
                            ...
                            #if (105 < event.jet_puppi_softdrop_jet1 < 200 and 45 < event.jet_puppi_softdrop_jet2 < 65):
                                #qVHP.Fill(event.MVV, event.weight)
                                #continue
                        #if event.jet_puppi_tau2tau1_jet2 <= 0.40:
                            #if (105 < event.jet_puppi_softdrop_jet2 < 200 and 45 < event.jet_puppi_softdrop_jet1 < 65):
                                #qVHP.Fill(event.MVV, event.weight)
                    ...
                    #============= try new qV sideband with W jet required to be in lower mass window between 20 and 65 GeV
                    #if ( event.jet_puppi_tau2tau1_jet1 <= 0.40 or event.jet_puppi_tau2tau1_jet2 <= 0.40):
                        #if event.jet_puppi_tau2tau1_jet1<= 0.40:
                            #if (45 < event.jet_puppi_softdrop_jet1 < 65):
                                #qVHP.Fill(event.MVV, event.weight)
                                #continue
                        #if event.jet_puppi_tau2tau1_jet2 <= 0.40:
                            #if (45 < event.jet_puppi_softdrop_jet2 < 65):
                                #qVHP.Fill(event.MVV, event.weight)
